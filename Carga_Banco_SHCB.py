...
# MAGIC
# MAGIC Este notebook:
# MAGIC 1. Lee el archivo CSV `pro_app.essenneg.Banco_DCV_paso1_22_shcb.csv` y lo carga en la tabla de paso `pro_app.essenneg.Banco_DCV_paso1_22_shcb`
# MAGIC 2. Lee el archivo CSV `pro_app.essenneg.Rutero_SHCB.csv` y lo carga en la tabla de paso `pro_app.essenneg.Banco_Rutero_paso1_22_shcb`
...
# MAGIC %md
# MAGIC ## 6. Escritura en tabla de paso pro_app.essenneg.Banco_DCV_paso1_22_shcb

# COMMAND ----------

target_table_dcv = "pro_app.essenneg.Banco_DCV_paso1_22_shcb"
...
# MAGIC %md
# MAGIC ## 7. Escritura en tabla de paso pro_app.essenneg.Banco_Rutero_paso1_22_shcb

# COMMAND ----------

target_table_rutero = "pro_app.essenneg.Banco_Rutero_paso1_22_shcb"
...
# MAGIC %md
# MAGIC ## 9. DELETE por data_date_part e INSERT en pro_business.essenexp.Banco_SHCB
...
spark.sql(f"""
    DELETE FROM pro_business.essenexp.Banco_SHCB
...
spark.sql(f"""
    INSERT INTO pro_business.essenexp.Banco_SHCB
    WITH max_acciones AS (
        SELECT MAX(Total_Acciones) AS max_total_acciones
        FROM pro_app.essenneg.Banco_DCV_paso1_22_shcb
    )
    SELECT
        ...
        ''                                   AS more_detail,                
        '{data_date_part}'
    FROM pro_app.essenneg.Banco_DCV_paso1_22_shcb DCV
    ...
    LEFT JOIN (
        ...
        FROM pro_app.essenneg.Banco_Rutero_paso1_22_shcb
        GROUP BY Rut_Accionista
    ) Ban_Rutero
        ON DCV.Rut = Ban_Rutero.Rut_Accionista
""")

print(f"INSERT completado en '{dest_table}' para data_date_part = '{data_date_part}'")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 10. Verificacion de la tabla destino
...
print(f"Registros en '{dest_table}' para data_date_part = '{data_date_part}': {insert_count}")
...
spark.sql(f"""
DECLARE OR REPLACE VARIABLE fecha_de_cierre STRING
""")

spark.sql(f"""
SET VARIABLE fecha_de_cierre = '{data_date_part}'
""")

spark.sql("""
DECLARE OR REPLACE VARIABLE fecha_master_tabla STRING
""")

spark.sql("""
SET VARIABLE fecha_master_tabla = (SELECT max(data_date_part) FROM pro_business.conrepexp.master_tabla
WHERE data_date_part <= fecha_de_cierre)
""")

print(f"fecha_de_cierre = {data_date_part}")
fecha_master_tabla_val = spark.sql("SELECT fecha_master_tabla AS val").collect()[0]["val"]
print(f"fecha_master_tabla = {fecha_master_tabla_val}")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 12. INSERT en pro_app.conrepneg.master_00051_parametria_automatica_satelite_22_shcb

# COMMAND ----------

spark.sql("""
INSERT INTO TABLE pro_app.conrepneg.master_00051_parametria_automatica_satelite_22_shcb
REPLACE WHERE data_date_part = fecha_de_cierre --viene del python
SELECT  C.empresa,
        C.cargabal,
        C.cuenta_contable,
        C.tipo_informacion,
    ...
    INNER JOIN (
        SELECT  counterparty_entity,
            ...
            WHERE data_date_part = fecha_master_tabla
    ) M
...
print("INSERT completado en 'pro_app.conrepneg.master_00051_parametria_automatica_satelite_22_shcb'")
...
data_date_part = fecha_de_cierre --viene del python
SELECT  C.reporting_soc,
    ...
    FROM (
        SELECT  empresa AS reporting_soc,
                ...
                sum(monto) AS monto,
                data_date_part
            FROM pro_app.conrepneg.master_00051_parametria_automatica_satelite_22_shcb
            WHERE data_date_part = fecha_de_cierre
            GROUP BY empresa,
                intergrupo,
                tipo_ajuste,
        ...
        SELECT  intergrupo AS shcode,
            ...
            FROM (
                SELECT  intergrupo,
                    ...
                    FROM (
                        SELECT  *,
                                sum(total) OVER() AS total_acciones,
                                '000000000000' AS ISIN
                            FROM pro_business.essenexp.Banco_SHCB --pro_business.conrepexp.mayores_accionistas
                            WHERE data_date_part = fecha_de_cierre
                    ) A
                    GROUP BY intergrupo,
                        agrupacion_de_accionistas,
...
SELECT  reporting_soc AS empresa,
        ...
        data_date_part, getdate() fecha_de_ejecucion
    FROM  pro_app.essenneg.master_00051_satelite_22_shcb
    WHERE data_date_part = fecha_de_cierre  --viene del python
""")
...
for tbl in [
    "pro_app.conrepneg.master_00051_parametria_automatica_satelite_22_shcb",
