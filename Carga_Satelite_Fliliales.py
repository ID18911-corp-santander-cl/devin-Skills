...
# MAGIC 2. Lo carga en la tabla de paso `pro_app.essenneg.csv_filial_paso1_22_shcb`
...
csv_path = f"/Volumes/pro_app/essenneg/motor/CSV_Filiales/Base_Filial_{fecha_proceso}.csv"
...
# MAGIC %md
# MAGIC ## 4. Escritura en la tabla pro_app.essenneg.csv_filial_paso1_22_shcb

# COMMAND ----------

target_table = "pro_app.essenneg.csv_filial_paso1_22_shcb"
...
# MAGIC %md
# MAGIC ## 6. DELETE por data_date_part en pro_business.essenexp.master_filiales_interfaz_satelite_22_shcb

# COMMAND ----------

dest_table = "pro_business.essenexp.master_filiales_interfaz_satelite_22_shcb"
...
# MAGIC %md
# MAGIC ## 7. Mapeo e INSERT append en pro_business.essenexp.master_filiales_interfaz_satelite_22_shcb
# MAGIC
# MAGIC | Campo Destino | Campo Origen |
# MAGIC |---|---|
# MAGIC | reporting_soc | Sociedad |
...
# MAGIC | isin | Literal: '000000000000' |
# MAGIC | ic | Literal: '152' |
# MAGIC | osha | Number_of_owned_shares |
# MAGIC | ownpi | Pct_ownership_per_issuance |
# MAGIC | votr | Pct_voting_rights |
# MAGIC | REPORTING_SOC\|COUNTERPARTY_SOC\|...\|VOTR | Concatenación de los 12 campos anteriores separados por '\|' |
...
from pyspark.sql.functions import lit, current_timestamp, date_format, from_utc_timestamp, concat_ws, coalesce as F_coalesce

df_paso = spark.table(target_table)

# Nombre del campo concatenado
concat_col_name = (
    "REPORTING_SOC|COUNTERPARTY_SOC|ADJUSTMENT_CODE|ID_COMB|"
    "AMOUNT|SHCODE|SHNAME|ISIN|IC|OSHA|OWNPI|VOTR"
)

df_mapped = df_paso.select(
    col("Sociedad").alias("reporting_soc"),
    ...
    concat_ws(
        "|",
        F_coalesce(col("Sociedad").cast("string"), lit("")),
        F_coalesce(col("Shareholder_entity_code").cast("string"), lit("")),
        F_coalesce(col("BIxxxxx").cast("string"), lit("")),
        F_coalesce(lit("B03;MC23;PR18"), lit("")),
        F_coalesce(col("AMOUNT").cast("string"), lit("")),
        F_coalesce(col("Shareholder_entity_code").cast("string"), lit("")),
        F_coalesce(col("Name_of_the_shareholder_entity").cast("string"), lit("")),
        F_coalesce(lit("000000000000"), lit("")),
        F_coalesce(lit("152"), lit("")),
        F_coalesce(col("Number_of_owned_shares").cast("string"), lit("")),
        F_coalesce(col("Pct_ownership_per_issuance").cast("string"), lit("")),
        F_coalesce(col("Pct_voting_rights").cast("string"), lit("")),
    ).alias(concat_col_name),
